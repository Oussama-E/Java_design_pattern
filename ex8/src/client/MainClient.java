		...
		try {
			...
			while (fichier.hasNextLine()) {
				try {
					...
					Generateur generateur = new ValidationVisa(new ValidationMasterCard(new ValidationDiscover(
							new ValidationDinersClub(new ValidationAmEx(null)))));
					CarteDeCredit carteDeCredit = generateur.traiterRequete(parts[0], dateExpiration, parts[2]);
